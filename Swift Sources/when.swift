...
public func when<T>(promises: [Promise<T>]) -> Promise<[T]> {
    ...
    for (index, promise) in enumerate(promises) {
        ...
        promise.catchVoid(body: rejecter)
    }
    return promise
}
...
public func when<U,V>(promise1: Promise<U>, promise2: Promise<V>) -> Promise<(U,V)> {
    ...
    promise1.catchVoid(body: rejecter)
    promise2.catchVoid(body: rejecter)
    return promise
}
...
private func when(promise1: Promise<Void>, # promise2: Promise<Void>) -> Promise<Void> {
    ...
    let _:Void = promise1.catchVoid(body: rejecter)
    let _:Void = promise2.catchVoid(body: rejecter)
    return promise
}
